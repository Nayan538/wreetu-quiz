...
@section('description', 'Location Type')
@extends('layout.app')
@section('content')
@section('content')
    <!-- CONTENT AREA -->
                        ...
                        <div class="breadcrumb-action justify-content-center flex-wrap">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">{{ trans('Location type') }}</li>
                                </ol>
                            </nav>
                        </div>
                        <div class="breadcrumb-main__wrapper">
                            <div class="action-btn mt-sm-0 mt-15">
                            @if(hasPermission('location_manager.location-types.create'))
            ...
            <div class="row">
                <div class="col-md-12" style="padding-bottom: 20px">
                    <div class="row" style="width: 100%">
                        <div class="col-md-6">
                            <h4 class="text-capitalize breadcrumb-title">{{ trans('Location type') }}</h4>
                        </div>
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="card mb-4">
                        <div class="card-body">
                            <table id="zero-config" class="table dt-table-hover" style="width:100%">
                                ...
                                <tbody>
                                    @csrf
                                    @foreach ($locationTypes as $key => $item)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <div class="btn-group btn-group-sm" role="group"
                                                    ...
                                                    @if(hasPermission('location_manager.location-types.update'))
                                                    <a href={{ $item->id }} class="btn btn-edit  btn-outline-warning"
                                                        data-name="{{ $item->name }}" data-code="{{ $item->code }}" data-status="{{$item->status}}"
                                                        data-action="{{ route('location_manager.location-types.update', $item->id) }}"
                                                        data-toggle="tooltip" data-placement="top" title="Edit"
                                                        data-bs-toggle="modal" data-bs-target="#editModal">
                                                        <i class="far fa-edit"></i>
                                                    </a>
                                                    @endif
                                                    @if(hasPermission('location_manager.location-types.destroy'))
                                                    <button type="button"
                                                        data-action="{{ route('location_manager.location-types.destroy', $item->id) }}"
                ...
                <div class="modal fade inputForm-modal" id="createModal" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog modal-md" role="document">
                        <div class="modal-content">
                            ...
                            <form action="{{ route('location_manager.location-types.store') }}" method="post">
                                @csrf
