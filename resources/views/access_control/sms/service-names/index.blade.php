                        ...
                        <div class="breadcrumb-action justify-content-center flex-wrap">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">{{ trans(' Service Name') }}</li>
                                </ol>
                            </nav>
                        </div>
                        <div class="breadcrumb-main__wrapper">
                            <div class="action-btn mt-sm-0 mt-15">
                                @if (hasPermission('sms.service-names.create'))
            ...
            <div class="row">
                <div class="col-md-12" style="padding-bottom: 20px">
                    <div class="row" style="width: 100%">
                        <div class="col-md-6">
                            <h4 class="text-capitalize breadcrumb-title">{{ trans('Service Names') }}</h4>
                        </div>
                    </div>
                    <x-error-alart />
                </div>
                    ...
                    <div class="card mb-4">
                        <div class="card-body">
                            <table id="zero-config" class="table dt-table-hover" style="width:100%">
                                ...
                                <tbody>
                                    @csrf
                                    @foreach ($serviceNames as $key => $item)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <div class="btn-group btn-group-sm" role="group"
                                                    ...
                                                    @if (hasPermission('sms.service-names.update'))
                                                        <a href={{ $item->id }} class="btn btn-edit  btn-outline-warning"
                                                            data-name="{{ $item->name }}" data-code="{{ $item->code }}" data-status="{{ $item->status }}"
                                                            data-action="{{ route('sms.service-names.update', $item->id) }}"
                                                            data-toggle="tooltip" data-placement="top" title="Edit"
                                                            data-bs-toggle="modal" data-bs-target="#editModal">
                                                            <i class="far fa-edit"></i>
                                                        </a>
                                                    @endif
                                                    @if (hasPermission('sms.service-names.destroy'))
                                                        <button type="button"
                                                            data-action="{{ route('sms.service-names.destroy', $item->id) }}"
                    ...
                    <div class="modal-dialog modal-md" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-header" id="createModalLabel">
                                <h5 class="modal-title">Add Service Name</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-hidden="true"></button>
                            </div>
                            <form action="{{ route('sms.service-names.store') }}" method="post">
                                @csrf
